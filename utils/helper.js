...
async function determineStatus(productId) {
    try {
        // Fetch the product with its brand and category populated to check brand/category status
        const product = await Product.findById(productId)
            .populate('brand')
            .populate('category')
            .lean();
        ...
        if (!brandIsListed) {
            console.log(`Product ${productId} has unlisted brand, marking as Discontinued`);
            return 'Discontinued';
        }

        if (!categoryIsListed) {
            console.log(`Product ${productId} has unlisted category, marking as Discontinued`);
            return 'Discontinued';
        }

        if (!product.isListed) {
            console.log(`Product ${productId} is not listed, marking as Discontinued`);
            return 'Discontinued';
        }

        if (product.quantity <= 0) {
            console.log(`Product ${productId} has quantity ${product.quantity}, marking as Out of stock`);
            return 'Out of stock';
        }
...
const updateProductsForBrand = async (brandId) => {
    try {
        ...
        for (let product of products) {
            const newStatus = await determineStatus(product._id, product.isListed, product.quantity);
